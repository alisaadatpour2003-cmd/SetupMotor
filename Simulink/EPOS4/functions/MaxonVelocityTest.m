...
if ~isfile(dllName)
    error(['CRITICAL: ', dllName, ' is missing. Please copy it from the Epos2Windows folder to this folder.']);
end
...
fprintf(fid, 'int __stdcall VCS_MoveToPosition(void* KeyHandle, unsigned short NodeId, long TargetPosition, int Absolute, int Immediately, unsigned int* pErrorCode);\n');
